...
from slambuc.alg.util import (induced_subtrees, par_inst_count, ibacktrack_chain, verify_limits, par_subchain_latency,
                              iclosed_subgraph, par_subgraph_cost)


def build_greedy_dag_mtx_model(dag: nx.DiGraph, root: int = 1, M: int = math.inf,
...
def build_dag_mtx_model(dag: nx.DiGraph, root: int = 1, M: int = math.inf,
                        L: int = math.inf, N: int = 1, cpath: set[int] = frozenset(),
                        delay: int = 1) -> tuple[lp.LpProblem, dict[int, dict[int, lp.LpVariable]]]:
    """
    #FIXME
    ...
    for (p, j), st_edges in induced_subtrees(dag, root):
        blk_mem = dag.nodes[j][MEMORY] * X[j][j]
        # Add coefficients for single node block [j]
        r_j, d_j, t_j = dag[p][j][RATE], dag[p][j][DATA], dag.nodes[j][RUNTIME]
        sum_cost += (r_j * (d_j + t_j) + sum(par_inst_count(r_j, js[RATE], N) * js[DATA]
                                             for js in dag.succ[j].values())) * X[j][j]
        if j in cpath:
            jc = next(filter(lambda c: c in cpath, dag.successors(j)), None)
            sum_lat += (d_j + t_j + (math.ceil(dag[j][jc][RATE] / (r_j * N)) * dag[j][jc][DATA]
                                     if jc else 0)) * X[j][j]
            if j != root:
                sum_lat += delay * X[j][j]
        ...
        for u, v in st_edges:
            ...
            r_v, d_v, t_v = dag[u][v][RATE], dag[u][v][DATA], dag.nodes[v][RUNTIME]
            # sum_cost += (r_v * (t_v - d_v) + sum(vs[RATE] * vs[DATA] for vs in tree.succ[v].values())) * X[v][j]
            sum_cost += ((par_inst_count(r_j, r_v, N) * (t_v - d_v) +
                          sum(par_inst_count(r_j, vs[RATE], N) * vs[DATA] for vs in dag.succ[v].values()))) * X[v][j]
            # u -> v edge on cpath
            if v in cpath:
                n_v *= math.ceil(r_v / (dag[next(dag.predecessors(u))][u][RATE] * N))
                vc = next(filter(lambda c: c in cpath, dag.successors(v)), None)
                w_v = math.ceil(dag[v][vc][RATE] / (r_v * N)) * dag[v][vc][DATA] if vc else 0
                sum_lat += n_v * (t_v - d_v + w_v) * X[v][j]
            # Add knapsack constraint for operative memory demand
            vj_sat = min(math.ceil(dag[u][v][RATE] / r_j), N)
            # Add only non-trivial memory constraint
            if vj_sat > 1:
...
def greedy_dag_partitioning(dag: nx.DiGraph, root: int = 1, M: int = math.inf,
                            L: int = math.inf, N: int = 1, cp_end: int = None,
                            delay: int = 1, solver: lp.LpSolver = None, timeout: int = None, **lpargs) -> T_RESULTS:
    """
    ...
    if not all(verify_limits(dag, cpath, M, L)):
        ...
        return INFEASIBLE
    model, X = build_greedy_dag_mtx_model(dag, root, M, L, N, cpath, delay)
...
def dag_partitioning(tree: nx.DiGraph, root: int = 1, M: int = math.inf, L: int = math.inf,
    ...
    if not all(verify_limits(tree, cpath, M, L)):
        # No feasible solution due to too strict limits
        return INFEASIBLE
    model, X = build_dag_mtx_model(tree, root, M, L, N, cpath, subchains, delay)
    status = model.solve(solver=solver if solver else lp.PULP_CBC_CMD(mip=True, msg=False, timeLimit=timeout, **lpargs))
    if status == lp.LpStatusOptimal:
        opt_cost, opt_lat = round(lp.value(model.objective), 0), round(lp.value(model.constraints[LP_LAT]), 0)
        return extract_subtrees_from_xmatrix(X), opt_cost, L + opt_lat if L < math.inf else opt_lat
